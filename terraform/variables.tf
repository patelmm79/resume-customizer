...
# Settings persistence storage provider
variable "storage_provider" {
  description = "Cloud storage provider for settings persistence. Choose 's3' for AWS S3 or 'gcs' for Google Cloud Storage. Set to empty string to disable settings persistence."
  type        = string
  default     = ""

  validation {
    condition     = contains(["", "s3", "gcs"], var.storage_provider)
    error_message = "storage_provider must be '', 's3', or 'gcs'."
  }
}
